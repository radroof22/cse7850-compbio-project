...
session = ort.InferenceSession("./proteinclip/project3d_1.onnx", sess_options=options)
...
with open("./data/structural/structural_embeddings_2000_5000.pkl", "rb") as f:
    embeddings_data = pickle.load(f)

# Convert to DataFrame (assuming the embeddings are stored in a dictionary-like structure)
df = pd.DataFrame(embeddings_data)
df = df[0:100]
input_array = np.stack(df["embedding"].to_numpy()).astype(np.float32)
...
# Save to file
plt.savefig("umap_embedding_comparison_pp.png", dpi=300)  # High-res PNG
