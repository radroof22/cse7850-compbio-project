...
# Load model once globally (like CLIENT)
HF_EMBEDDING_MODELS = {
...
# Load a Hugging Face chat-style model (once globally)
HF_CHAT_MODEL = "meta-llama/Llama-2-7b-chat-hf"
# Options: "mistralai/Mistral-7B-Instruct-v0.2", "meta-llama/Llama-2-7b-chat-hf"
tokenizer = AutoTokenizer.from_pretrained(HF_CHAT_MODEL)
model = AutoModelForCausalLM.from_pretrained(HF_CHAT_MODEL, torch_dtype=torch.float16, device_map="auto")
generator = pipeline("text-generation", model=model, tokenizer=tokenizer)


embedding_cache = FanoutCache(
    directory=os.path.expanduser("~/scratch/.cache/hf_embed"),
    timeout=0.1,
    size_limit=int(2e9),  # Size limit of cache in bytes; 8GB
    eviction_policy="least-recently-used",
)
keyword_cache = FanoutCache(
    directory=os.path.expanduser("~/scratch/.cache/hf_keyword"),
    timeout=0.1,
    size_limit=int(2e9),  # Size limit of cache in bytes; 5GB
    eviction_policy="least-recently-used",
)


def sanitize_text(ft: str, max_len: int = 512) -> str:
...
def get_hf_embedding(s: str, model: str = "thenlper/gte-large") -> np.ndarray:
    """Get the Hugging Face embedding for the given string s."""
    s = s.strip()
    
    # For retrieval-style models, add 'query:' prefix if needed
    # TODO: Figure out if this 'query thing is really needed
    if "e5" in model or "bge" in model:
        s = f"query: {s}"

    embed_model = HF_EMBEDDING_MODELS[model]
    embedding = embed_model.encode([s], normalize_embeddings=True)[0]
    return np.array(embedding)

@keyword_cache.memoize(typed=True, name="gptprotein-keywords")
def get_hf_keywords(text: str, addtl_sys_prompt: str = "") -> List[str]:
    """Extract keywords from a block of text using a Hugging Face model."""
    
    # Build a plain prompt (chat-style)
    full_prompt = (
        ...
        + "\n\n"
        + "Text:\n"
        + sanitize_text(text, max_len=2000)
        + "\n\nKeywords:"
    ).strip()

    # Generate
    output = generator(full_prompt, max_new_tokens=64, do_sample=False)[0]["generated_text"]

    # Extract only the keyword list after "Keywords:"
    keyword_part = output.split("Keywords:")[-1]
    return [s.strip() for s in keyword_part.split(",") if s.strip()]
...
if __name__ == "__main__":
    ...
    #     ),
    # )
    e = get_hf_embedding(SAMPLE_TEXT_1)
